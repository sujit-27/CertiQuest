...
export const createQuiz = createAsyncThunk(
  ...
  async ({ title, category, difficulty, noOfQuestions, token, createdBy }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post('https://certiquest-n9al.onrender.com/api/quiz/create', body, {
...
export const submitQuiz = createAsyncThunk(
  ...
  async ({ quizId, answers,isSignedIn, token, userId }, { rejectWithValue }) => {
    try {
      if (!isSignedIn) return rejectWithValue('Not signed in');
      const response = await axios.post(
        `https://certiquest-n9al.onrender.com/api/quiz/${quizId}/submit?userId=${encodeURIComponent(userId)}`,
        { quizId,answers },
        { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' } }
      );
      return response.data;
